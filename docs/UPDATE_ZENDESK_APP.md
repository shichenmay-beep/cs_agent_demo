...
- **前端**：侧栏顶部右侧小字 **「v1.0.2」**（或当前 manifest 里的 version）。上传新包后这里变成新版本号，说明**前端包已加载**。
- **后端**：同一行会显示 **「后端 v1.0.2」**（或当前后端 `BACKEND_VERSION`）。若显示「后端 未连接」，说明 Backend URL 不对或后端未部署/未启动；若显示「后端 v1.0.2」且与预期一致，说明**当前连上的后端就是最新部署**。

若前端仍是旧号，多半是缓存或没用新 zip 更新；若后端一直「未连接」或版本不对，检查 Zendesk 里填的 Backend URL 和 Railway/VM 部署是否成功。
